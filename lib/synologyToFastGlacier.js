var fs = require('fs'),
    ...
    DATA_FORMAT = '<m><v>2</v><p>%s</p><lm>%s</lm></m>',
    AWS_CREATION_DATE_FORMAT = 'DD/MM/YYYY h:mm:ss A',
    FASTGLACIER_DATE_FORMAT = 'YYYYMMDD[T]HHmmss[Z]';
                    ...
                    if (err !== null) {
                        ...
                        outstream.write(prefix, 'utf8', function () {
                            ...
                            readStream.pipe(JSONStream.parse('ArchiveList.*'))
                                .pipe(es.map(function (data, internalCallback) {
                                        ...
                                        if (err !== null) {
                                            ...
                                            var thePath = path.join(row.shareName, row.basePath),
                                                ...
                                                date = moment(data.CreationDate, 
                                                    AWS_CREATION_DATE_FORMAT)
                                                    .format(FASTGLACIER_DATE_FORMAT);

                                            data.ArchiveDescription = util
                                                .format(DATA_FORMAT, base64, date);
                                            count++;
